...
export const TaskProjectsListScreen: React.FC<any> = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'bottom']}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          defaultProject ? (
            <ProjectCard
              ...
              onPress={() => navigation.navigate('ProjectTasks', { project: defaultProject })}
        ...
        renderItem={({ item }) => (
          <ProjectCard project={item} onPress={() => navigation.navigate('ProjectTasks', { project: item })} taskCount={item.tasks?.length || 0} />
        )}
        ListEmptyComponent={
          !defaultProject ? (
      ...
      <ProjectFormModal visible={showCreateModal} onClose={() => setShowCreateModal(false)} feature={FeatureEnum.Task} />
    </SafeAreaView>
  );
};
