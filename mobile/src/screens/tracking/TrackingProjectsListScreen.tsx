...
export const TrackingProjectsListScreen: React.FC<any> = ({ navigation }) => {
  const colors = useThemeColors();
  const [showCreateModal, setShowCreateModal] = useState(false);
  const { data: projects, isLoading, refetch } = useProjects(FeatureEnum.Tracking, false);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'bottom']}>
        ...
        ListHeaderComponent={
          defaultProject ? (
            <ProjectCard
              ...
              onPress={() => navigation.navigate('ProjectTrackings', { project: defaultProject })}
              taskCount={defaultProject.trackings?.length || 0}
              isDefault
            />
          ) : null
        }
        renderItem={({ item }) => (
          <ProjectCard project={item} onPress={() => navigation.navigate('ProjectTrackings', { project: item })} taskCount={item.trackings?.length || 0} />
        )}
        ListEmptyComponent={
          !defaultProject ? (
      ...
      <ProjectFormModal visible={showCreateModal} onClose={() => setShowCreateModal(false)} feature={FeatureEnum.Tracking} />
    </SafeAreaView>
  );
};
